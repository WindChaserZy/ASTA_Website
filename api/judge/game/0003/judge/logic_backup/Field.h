...
public:
    ...
    //�����ƶ������Ӹ����ڵĽ�ʬ
    void zombie_append(zombies* zombie) {
        this->zombie.push_back(zombie);
    }
    //��ֲ�ﺯ��
    ...
    //���ý�ʬ����
    bool PlaceZombie(zombies* Zombie) {
        if (Zombie == nullptr)
            return false;
        this->zombie.push_back(Zombie);
        this->exists_zombie = true;
    ...
    //�õ�Ŀǰ�����ڵĽ�ʬ
    vector<zombies*> GetThisGridZombie() {
        // if(this->zombie.empty())
        //     return NOZOMBIE;
        return zombie;
    }
...
public:
    ...
    bool PlaceZombie(int y, zombies* zombie) {
        bool IsPlaced = grids[y][this->columns - 1].PlaceZombie(zombie);
        if (IsPlaced)
            zombies_num++;
        return IsPlaced;
    }
    ...
    void Zombie_move(int t) {
        for (int i = 0; i < rows; i++)
        {
            ...
            for (int j = 0; j < columns; j++)
            {
                ...
                vector<zombies*> zombies = grids[i][j].GetThisGridZombie();
                int m = 0;
                //�������õ����Ǹ�����
                for (int k = 0; k < zombies.size(); k++)
                {
                    ...
                    int type = zombies[k]->getZombie_Type();
                    if (type == POLEVAULT)
                    {
                        PolevaultZombies* temp = dynamic_cast<PolevaultZombies*> (zombies[k]);
                        if (grids[i][j].get_exists_plant() && temp->get_Ifpole())
                        {
                            ...
                            if (j > 0)
                            {
                                grids[i][j - 1].zombie_append(temp);
    ...
    void Zombies_check(int t) {//��齩ʬѪ����ɾ����Ѫ����ʬ
        ...
        for (int i = 0; i < this->rows; i++) {
            for (int j = 0; j < this->columns; j++) {
                vector<zombies*> this_zombies = this->grids[i][j].GetThisGridZombie();//��õ�ǰ���ӵĽ�ʬ����
                int m = 0;
                for (int k = 0; k < this_zombies.size(); k++) {
                    zombies* this_zombie = this_zombies[k];
        ...
        for (int i = 0; i < this->rows; i++) {
            for (int j = 0; j < this->columns; j++) {
                ...
                if (p!=nullptr && p->get_able_attack())//�ܹ�����
                {
                    ...
                    case WINTERPEASHOOTER: {
                        for (int k = j; k < this->columns; k++) {
                            if (this->grids[i][k].get_exists_zombie()) {
                                vector<zombies*> zombies = this->grids[i][k].GetThisGridZombie();//��ȡ�ø�Ľ�ʬ
                                int nearest = 0;
                                for (int iter = 0; iter < zombies.size(); iter++) {
                                    if (zombies[iter]->getfx() < zombies[nearest]->getfx())
                                        nearest = iter;
                                }
                                p_attack_z(p, zombies[nearest], filename);
                                zombies[nearest]->frozen();
                                break;//����������ѭ��
                            }
                        }
                        break;
                    ...
                    case PEPPER: {
                        ...
                        for (int k = 0; k < this->columns; k++) {
                            if (this->grids[i][k].get_exists_zombie()) {
                                vector<zombies*> zombies = this->grids[i][k].GetThisGridZombie();//��ȡ�ø�Ľ�ʬ
                                for (int iter = 0; iter < zombies.size(); iter++) {
                                    p_attack_z(p, zombies[iter], filename);
                                }
                            }
                        }
                        break;
                    ...
                    case SQUASH: {
                        vector<zombies*> zombies = this->grids[i][j].GetThisGridZombie();
                        if (zombies.size() != 0) {
                            for (int iter = 0; iter < zombies.size(); iter++) {
                                p_attack_z(p, zombies[iter], filename);
                            }
    ...
    void z_condition_p(string filename) {
        for (int i = 0; i < this->rows; i++) {
            for (int j = 0; j < this->columns; j++) {
                vector<zombies*> this_zombie = this->grids[i][j].GetThisGridZombie();//��ȡ�ø�Ľ�ʬ 
                plants* p = this->grids[i][j].GetThisGridPlant();
                ...
                if (!this_zombie.empty() && p != nullptr)//ֲ��ͽ�ʬ��һ�����������ܹ�����
                {
                    for (int h = 0; h < this_zombie.size(); h++) {
                        ZombieType zombie_type = this_zombie[h]->getZombie_Type();
                        z_attack_p(this_zombie[h], p, filename);
                    }
                }//endif   
            }
        }
