                ...
                for (int i = 0; i < ROW; i++)
                {
                    if (LeftLines[i] == 0)
                    {
                        for (int j = 0; j < COLUMN; j++)
                        {
                            ...
                            else if (k > 0 && (have_type_of_zombies(Zombies[i][j], POLEVAULT) || have_type_of_zombies(Zombies[i][j], BUCKET)) && Sun < 400 && time < 500 && j < 4)
                            {
                                int start = 0;
...
public:
    ...
    double plant_cost(int row, int **plants_num_format, double *plants_para, int kind)
    {
        ...
        case SLED:
            ...
            break;
        case GARGANTUAR:
            if (plants_num_format[row][WINTERPEASHOOTER - 1] > 1)
            {
                plants_para[WINTERPEASHOOTER - 1] *= (plants_num_format[row][WINTERPEASHOOTER - 1] + 1);
            }
        }
    ...
    */
    ...
    void value_normal()
    {
        ...
        double plants_para[PLANT_KIND] = {8, -8, -2, -2, -100, 5};
    ...
    void value_bucket()
    {
        ...
        //遍历plant
        int plants_num_format[ROW][PLANT_KIND] = {{0}};
        double plants_para[PLANT_KIND] = {6, 3, 2, 3, -100, -4};
    ...
    void value_sled()
    {
        ...
        //考虑Sun
        double sun_baseline = 100, sun_sub = 0.5;
    ...
    void value_gargantuar()
    {
        ...
        //遍历plant
        int plants_num_format[ROW][PLANT_KIND] = {{0}};
        double plants_para[PLANT_KIND] = {0, 100, 2, 5, -100, -10}; //偏好建国 and 冰豌豆
        int **sum_plants_per_row = this->sum_plants_per_row();
        for (int i = 0; i < ROW; i++)
        {
        ...
        //考虑Sun
        double sun_baseline = 550, sun_sub = 0.6;
...
extern "C" _declspec(dllexport) void player_ai(IPlayer *player)
{
    ...
    if (Type == 1)
    {
        ...
        {
            player->PlaceZombie(POLEVAULT, 3);
            player->PlaceZombie(NORMAL, 4);
            player->PlaceZombie(BUCKET, 5);
        }
    }
}
